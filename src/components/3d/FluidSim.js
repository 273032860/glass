...
import { vec2, vec3, float, sub, mul, add, min, uv, texture, Fn } from 'three/tsl';
import { fbm } from './fbm';
...
export default class FluidSim {
  ...
  #createFluidShader() {
    ...
    return Fn(() => {
      ...
      // Fade back to white
      return min(vec3(1.0), add(combined, vec3(0.008))); //调整流体延长时间，默认0.015
    })();
  }

  get texture() {
    return this.maskNode.value;
  }
